...
public abstract class Profile<R extends ReportsGeneExpression,
    ...
    public long getSpecificity(Collection<R> conditions) {
        List<String> dataColumnDescriptorIds = conditions.stream().map(ReportsGeneExpression::getId).collect(toList());
    ...
    public double getMaxExpressionLevelOn(Collection<R> conditions) {
        double expressionLevel = 0D;

        for (R condition : conditions) {
    ...
    public double getAverageExpressionLevelOn(Collection<R> conditions) {
    ...
    public void add(R condition, E expression) {
        expressionsByCondition.put(condition.getId(), expression);
    }

    public E getExpression(R condition) {
