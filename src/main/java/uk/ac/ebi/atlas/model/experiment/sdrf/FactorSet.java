...
public class FactorSet implements FactorGroup {
    ...
    @Override
    public int compareTo(@NotNull FactorGroup other) {
    ...
    public FactorGroup withoutTypes(Collection<String> factorTypes) {
        ...
        factorsByType.keySet().stream()
                .filter(type -> !factorTypes.contains(type))
                .forEach(type -> result.put(type, factorsByType.get(type)));
