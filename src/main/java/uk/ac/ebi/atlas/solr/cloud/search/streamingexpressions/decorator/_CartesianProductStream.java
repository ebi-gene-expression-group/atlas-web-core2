/*
 * This class is the CartesianProductStream as found in SolrJ with an added TupleStream-based constructor so that we
 * can wrap it in a CartesianProductStreamBuilder. The constructor, unlike the StreamExpression-based one, doesn’t
 * support the order parameter (although one could be added without much effort) but, most significantly, it doesn’t
 * support evaluators, only field names. However, that’s OK because it caters to our needs.
 */
...
public class _CartesianProductStream extends TupleStream implements Expressible {
  ...
  public _CartesianProductStream(TupleStream stream,
                                 Collection<String> fieldNames,
                                 @Nullable StreamComparator orderBy) {
    evaluators = new ArrayList<>();
    for (String fieldName: fieldNames) {
      StreamEvaluator evaluator = new FieldValueEvaluator(fieldName);
      evaluators.add(new NamedEvaluator(fieldName, evaluator));
    }
  ...
  public _CartesianProductStream(StreamExpression expression, StreamFactory factory) throws IOException {
    String functionName = factory.getFunctionName(getClass());

    // grab all parameters out
    List<StreamExpression> streamExpressions =
            factory.getExpressionOperandsRepresentingTypes(expression, Expressible.class, TupleStream.class);
    ...
    if (expression.getParameters().size() !=
            streamExpressions.size() + evaluateAsExpressions.size() + (null == orderByExpression ? 0 : 1)) {
      throw new IOException(
              String.format(
                      Locale.ROOT, "Invalid %s expression %s - unknown operands found", functionName, expression));
    }
