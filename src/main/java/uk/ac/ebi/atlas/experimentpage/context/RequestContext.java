...
public abstract class RequestContext
        ...
        implements ProfileStreamOptions<R> {
    ...
    public List<R> getAllDataColumns() {
        return ImmutableList.copyOf(experiment.getDataColumnDescriptors());
    }

    protected Stream<R> dataColumnsToBeReturned() {
        final Collection<String> selectedIds = requestPreferences.getSelectedColumnIds();

        Predicate<R> keepColumns = selectedIds.isEmpty() ?
    ...
    public List<R> getDataColumnsToReturn() {
        if (experiment.getDisplayDefaults().isColumnOrderPreserved()) {
            return dataColumnsToBeReturned().collect(toList());
        }

        return dataColumnsToBeReturned()
                        .sorted(Comparator.comparing(this::displayNameForColumn))
                        .collect(toList());
    }

    public abstract String displayNameForColumn(R dataColumnDescriptor);
