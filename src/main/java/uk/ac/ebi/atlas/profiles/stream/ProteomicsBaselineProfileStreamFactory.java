...
public class ProteomicsBaselineProfileStreamFactory
    ...
    @Override
    public ObjectInputStream<BaselineProfile>
           create(BaselineExperiment experiment,
                  BaselineProfileStreamOptions<ExpressionUnit.Absolute.Protein> options,
            ...
            for (int i = 2; i < headers.length; i++) {
                ...
                if (s.endsWith(AGREED_POSTFIX_FOR_DATA_COLUMNS)) {
                    AssayGroup assayGroup =
                            experiment.getDataColumnDescriptor(s.replace(AGREED_POSTFIX_FOR_DATA_COLUMNS, ""));
                    Validate.notNull(
                            assayGroup, MessageFormat.format("Unknown identifier in position {0}: {1}", i, s));
                    b.put(i, assayGroup);
                }
            }
