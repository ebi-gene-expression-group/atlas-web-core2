...
public class MicroarrayProfileStreamFactoryTest {
    ...
    public void twoRowsWithOneProfileEach() {
        List<Contrast> contrasts = generateRandomContrasts(1, true);
        ...
        String header =
                Joiner.on("\t").join(
                        new String[] {
                                ...
                                "gene name",
                                "design element",
                                contrasts.get(0).getId() + ".p-value",
                                contrasts.get(0).getId() + ".t-stat",
                                contrasts.get(0).getId() + ".fold-change"});
        String line1 =
                Joiner.on("\t").join(
    ...
    public void oneRowWithTwoProfilesEach() {
        List<Contrast> contrasts = generateRandomContrasts(2, true);
        ...
        String header =
                Joiner.on("\t").join(
                        new String[] {
                                ...
                                contrasts.get(0).getId() + ".t-stat",
                                contrasts.get(0).getId() + ".fold-change",
                                contrasts.get(1).getId() + ".p-value",
                                contrasts.get(1).getId() + ".t-stat",
                                contrasts.get(1).getId() + ".fold-change"});
    ...
    public static GeneProfilesList<MicroarrayProfile> loadProfiles(List<Contrast> contrasts,
        ...
        MicroarrayExperiment experiment =
                new MicroarrayExperimentBuilder()
                        .withExperimentType(MICROARRAY_1COLOUR_MRNA_DIFFERENTIAL)
                        .withExperimentAccession("accession")
                        .withSamples(contrasts)
                        .withArrayDesigns(ImmutableList.of(ArrayDesign.create("array-design-accession")))
                        .build();
